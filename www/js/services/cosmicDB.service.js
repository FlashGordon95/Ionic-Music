    ...
    var database={
        ...
        getArtists: function(){
            ...
            return $cordovaSQLite.execute(this.db,query, []).then(function(res) {
                console.log("START 2 getArtists ");
                var artists=new Array(res.rows.length);
                for (var i=0; i < res.rows.length; ++i){
                    artists[i]=res.rows.item(i);
                    console.log("loop >>>"+res.rows.item(i));
                }
                return artists;
            });
        },
       ...
       getAllTitles:  function(){
                console.log("2 getAllTitles");
            // Get all titles from artist
            var query= "SELECT title.name AS name, title.id AS id, title.like AS like, title.track AS track, title.path AS path,"+
                " artwork.file_name AS artwork, album.name AS album,"+
                " album.id AS albumId , arti.artist AS artist FROM"+
                " (SELECT  artist.name AS artist, artist.id FROM artist ) arti INNER JOIN"+
                " album ON album.artist = arti.id INNER JOIN"+
                " title ON title.album = album.id INNER JOIN"+
                " artwork ON album.artwork = artwork.id ORDER BY album.name,title.track";
            // return $cordovaSQLite.execute(this.db,query, []).then(function(res) {
            //     console.log("3  return $cordovaSQLite.execute");
            //     var artists=new Array(res.rows.length);
            //     for (var i=0; i < res.rows.length; ++i){
            //         artists[i]=res.rows.item(i);
            //         console.log("loop >>>"+res.rows.item(i));
            ...
            return $cordovaSQLite.execute(this.db,query, []).then(function(res) {
                ...
                var i0=0;
                console.log("3  return $cordovaSQLite.execute");
                console.log("######  i0 at 1 = "+i0);



                while (i<res.rows.length){
                    var currentAlbumId=res.rows.item(i).albumId;
                    //titles.push({ name: res.rows.item(i).album, id : res.rows.item(i).albumId, artwork : res.rows.item(i).artwork});//isSeparator : true,
                    i0=i;
                    //console.log(">>>>>>>>>>titles 1"+titles);
                    console.log("@@@@@  i0 at 2= "+i0);
                    ...
                    while (i<res.rows.length && res.rows.item(i).albumId==currentAlbumId){
                        ...
                        item.first = (i===i0);
                        viewPlaylist[i]=item;
                        console.log(">>>>>>>>>>>>>> i0 at 3= "+i0);    
                        titles.push(item);
                         //console.log(">>>>>>>>>>>>>>>>>>>>titles 2"+titles);
                        i++;   // counter
                         console.log("5  while...loop");
                    }
                }
                return {titles : titles, playlist : viewPlaylist}; // playlist : viewPlaylist For the player service
            },function(err){
                console.log('error');
                console.dir(err);
            });
        ...
        getArtistId: function(artistName){
            ...
            getArtistCover: function(artistId){
                var dbService=this;
                return $cordovaSQLite.execute(dbService.db, "SELECT * FROM artist WHERE id=?", [artistId.toLowerCase()]).then(function(res) {
                    // if (res.rows.length>0){
                    //     console.log("YES ");
                    //     return res.rows.item(0).id;
                    // } else {
                    //     console.log("no results or err ");
                    //     return 0;
                    // }
