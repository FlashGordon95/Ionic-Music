// Manage directories
angular.module('cosmic.controllers').controller('ManageDirectoriesCtrl', function( $scope,deviceFS,cosmicDB,$cordovaToast,cosmicConfig,$ionicLoading, $ionicModal,$timeout, fileFactory, $localstorage,$ionicListDelegate) {
     console.log("start $ionicLoading");
    ...
    $scope.scanDirectory = function(directory){
        if ($scope.isScanning){
            ...
            deviceFS.scanMusicFolder(directory.nativeURL).then(function(res){
             ...
             // $ionicLoading.hide();
             // console.log("End $ionicLoading");
        }
    };
    $scope.ShowSwipeInfo = function(){
         $cordovaToast.showShortTop('Swipe left to remove or scan.');
    }
    ...
    fs.getEntriesAtRoot().then(function(result) {
        ...
        console.log("files results ="+ JSON.stringify(fs.getEntriesAtRoot()));
         $scope.files = result;
    ...
    $scope.openModal = function() {
        ...
        $ionicModal.fromTemplateUrl('templates/file-browser-modal.html', {
            ...
            $cordovaToast.showShortTop('Tap and Hold (long press) to select folder');
            $scope.modal = modal;
            if ($scope.isScanning){
                $cordovaToast.showShortTop('Please wait for the first scan to be finished');
            } else {
                $cordovaToast.showShortBottom('Scan has finished');
